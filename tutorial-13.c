...
static void
send_seek_event(CustomData13 *data) {
    gint64 position;
    GstEvent *seek_event;
...
static gboolean
handle_keyboard(GIOChannel *source, GIOCondition cond, CustomData13 *data) {
    gchar *str = NULL;
...
int main13(int argc, char *argv[]) {
    ...
    data.pipeline =
            gst_parse_launch
            ("playbin uri=file:///C:/gstreamer/1.0/mingw_x86/bin/sintel_video.mkv",
                     NULL);
